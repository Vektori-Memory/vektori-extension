// Grok Content Script - Vektori Memory Extension
...
if (window.vektoriExtensionLoaded) {
  ...
  function getInputValue() {
    ...
    for (const selector of selectors) {
      ...
      if (inputElement) {
        if (inputElement.tagName.toLowerCase() === "div" || inputElement.tagName.toLowerCase() === "p") {
    ...
    observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'childList') {
          ...
          const buttonContainer = document.querySelector('div[class="rounded-t-[var(--border-t-radius)] rounded-b-[var(--border-b-radius)] query-bar group z-10 bg-surface-l1 ring-border-l1 hover:ring-border-l1 focus-within:ring-border-l1 hover:focus-within:ring-border-l1 relative w-full overflow-hidden @container/input shadow-sm max-w-breakout ring-1 ring-inset focus-within:ring-1 pb-0 shadow-black/5"]')
  ...
  function addMemoryButton(sendBtn) {
    ...
    memoryBtn.style.fontSize = '1.1em';
    memoryBtn.style.marginRight = '7px';
    memoryBtn.style.transform = 'translateY(-2px)';


    //then we insert the next to send button we have created it 
    const voiceDiv = document.querySelector('div.ms-auto.flex.flex-row.items-end.gap-1 > div[class=""]');
    if (voiceDiv) {
      voiceDiv.parentNode.insertBefore(memoryBtn, voiceDiv);
    } else {
  ...
  class BatchChatSaver {
    ...
    getCurrentConvoId() {
      // Grok URL formats: 
      // /c/{convoId} or /chat/{convoId}
      const url = window.location.href;
      const match = url.match(/\/(?:c|chat)\/([a-zA-Z0-9\-]+)/);
      if (match) {
        console.log(`[BatchChatSaver] Found Grok convo ID: ${match[1]}`);
        return match[1];
      }
      console.log('[BatchChatSaver] Could not extract Grok convo ID from URL:', url);
      return null;
    }
    ...
    observeNewMessages() {
      ...
      this.messageObserver = new MutationObserver((mutations) => {
          ...
          if (mutation.addedNodes.length > 0) {
            for (const node of mutation.addedNodes) {
              if (node.nodeType === Node.ELEMENT_NODE) {
                // Grok message selectors: <p> tags with white-space style OR .markdown elements
                const isMessage = node.matches && (
                  node.matches('p[style*="white-space"]') ||
                  node.querySelector('p[style*="white-space"]') ||
                  node.matches('.markdown') ||
                  node.querySelector('.markdown')
                );
                if (isMessage) {
                  console.log('[BatchChatSaver] New message detected in DOM!');
                  return true;
                }
              }
            }
        ...
        if (hasRelevantChanges) {
          console.log('[BatchChatSaver] Triggering debounced message check...');
    ...
    async checkForNewMessages() {
      console.log('[BatchChatSaver] checkForNewMessages() called');
      ...
      try {
        const convoId = this.getCurrentConvoId();
        console.log(`[BatchChatSaver] Current convo ID: ${convoId}`);

        if (convoId !== this.currentConvoId) {
          console.log('[BatchChatSaver] Convo ID changed, switching...');
          await this.onChatSwitch();
        }

        if (!this.currentConvoId) {
          console.log('[BatchChatSaver] No current convo ID, exiting');
          this.isProcessing = false;
          return;
        }

        console.log('[BatchChatSaver] Calling parser...');
        const chatData = this.parser();

        if (!chatData || !chatData.messages) {
          console.log('[BatchChatSaver] Parser returned no data');
          this.isProcessing = false;
          return;
        }
        ...
        console.log(`[BatchChatSaver] Parser returned ${allMessages.length} messages, last count was ${lastCount}`);
        ...
        if (allMessages.length > lastCount) {
          ...
          console.log(`[BatchChatSaver] Current batch size: ${this.messageBatch.length}`);
          ...
          console.log('[BatchChatSaver] No new messages found');
        }
      } catch (error) {
  ...
  async function initializeBatchSaver() {
    ...
    if (!batchSaver) {
      batchSaver = new BatchChatSaver('grok', grokChatParser);
      batchSaver.start();
  ...
  async function checkForPendingContext() {
    try {
      ...
      if (!pendingContext || pendingContext.destination !== 'grok') return;
      ...
      if (inputElement.tagName.toLowerCase() === 'div') { inputElement.innerHTML = pendingContext.context.replace(/\n/g, '<br>'); }
