window.geminiChatParser = function() {
    const messages = []
    ...
    const userMessages = document.querySelectorAll('[class^="user-query-bubble-with-background"]');
    const assistantMessages = document.querySelectorAll('[class^="response-content ng-tns-"]');
    ...
    userMessages.forEach((userEl) => {
        let content = parseUserMessage(userEl);
...
function parseUserMessage(messageEl) {
    ...
    const spanElement = messageEl.querySelector('span');
    if (spanElement) {
        return spanElement.textContent.trim();
    }
    // Fallback to the entire element's text content
...
{
    // Extract convo_id from URL: gemini.google.com/app/{convoId}
